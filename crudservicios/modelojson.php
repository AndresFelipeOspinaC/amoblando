...
class Datos extends Database
{
    ...
    {
        $stmt = Database::getconectar()->prepare("SELECT document_type,document,username,lastname,id_roll,gender,email,phone,address,password,confirm_password from $tabla
        join rol on rol.id_roll = $tabla.id_roll
        where email = :email");
  ...
  */
    ...
    public function createProductoModel($datosModel, $tabla)
    {
        ...
        $stmt = Database::getconectar()->prepare("INSERT into $tabla(id_prd,prd_name,descrip,price,stock,id_category,id_subca)
        values (:id_prd,:prd_name,:descrip,:price,:stock,:id_category,:id_subca)");
        $stmt->bindParam(":id_prd", $datosModel["id_prd"], PDO::PARAM_STR);
        $stmt->bindParam(":prd_name", $datosModel["prd_name"], PDO::PARAM_STR);
        ...
        $stmt->bindParam(":id_category", $datosModel["id_category"], PDO::PARAM_STR);
        $stmt->bindParam(":id_subca", $datosModel["id_subca"], PDO::PARAM_STR);
        if ($stmt->execute()) {
            return true;
        } else {
            return false;
    ...
    public function mostrarProductos(){
      ...
      $stmt = Database::getconectar()->prepare("SELECT id_prd, prd_name, Imagen_Producto,
  ...
  $stmt->execute();
          ...
          $stmt->bindParam(":id_prd", $id_prd,PDO::PARAM_STR);
    ...
    public function editarproducto($id_prd,$tabla){
    $stmt = Database::getconectar()->prepare("SELECT id_prd, prd_name, Imagen_Producto,
...
return $stmt->fetchAll();
    ...
    public function volverImagen($id_prd,$tabla){
...
if($datosModel["Imagen_Producto"] == "../administrador/fotos/".$datosModel["IMG"])

 {
    ...
    $MostrarImagen= $VolverImagen->volverImagen($datosModel["id_prd"],"producto");
...
$stmt->bindParam(":id_prd", $datosModel["id_prd"],PDO::PARAM_STR);
 $stmt->bindParam(":prd_name", $datosModel["prd_name"],PDO::PARAM_STR);
    ...
    public function deleteProductoModel($id_prd, $tabla){

    $stmt = Database::getconectar()->prepare("DELETE  FROM $tabla WHERE id_prd=:id_prd");
